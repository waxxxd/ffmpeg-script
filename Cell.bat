...
REM 
SET AUDIO_CHANNELS=-ac 1

FOR %%A IN (%*) DO (
	mkdir "%LOCATION%"

	echo ffmpeg -i %%A %TITLE% -y %VIDEO% %AUDIO% %TRACK% %START% %DURATION% %AUDIO_FILTER% %AUDIO_CHANNELS% %VIDEO_FILTER% "%LOCATION%\%FILENAME%"
	ffmpeg -i %%A %TITLE% -y %VIDEO% %AUDIO% %TRACK% %START% %DURATION% %AUDIO_FILTER% %AUDIO_CHANNELS% %VIDEO_FILTER% "%LOCATION%\%FILENAME%"
